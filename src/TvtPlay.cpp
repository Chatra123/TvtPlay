...
#include "Sn_CyclePopMenu6.h"
namespace CycPop = Snow::CycPop6;
#include "TvtPlay.h"
...
/*snow test*/
#define WM_TS_SEEK_AND_PREVIEW     (WM_APP + 12)
...
CTvtPlay::CTvtPlay()
	...
	, m_fBackup_AlwaysOnTop(false)
	, m_HaltCount_SetAlwaysOnTop(1)
	, m_fIsPause_byDriverChanged(false)
	, m_fAutoPlay(false)
#ifdef EN_SWC
...
bool CTvtPlay::Initialize()
{
	...
	// ステータス項目を登録
	{
		...
		item.Style = TVTest::STATUS_ITEM_STYLE_FORCEFULLROW;
		item.ID = 2;
		item.pszIDText = L"2TvtPlayStatus2";
		item.pszName = L"2TvtPlayステータスバー2";
		item.MinWidth = 0;
		item.MaxWidth = -1;
		item.DefaultWidth = TVTest::StatusItemWidthByFontSize(15);
		item.MinHeight = 0;
...
void CTvtPlay::LoadSettings()
{
	...
	int iniVer = 0;
	{
		...
		/*Snow*/
		GetBufferedProfileString(pBuf, TEXT("Snow_CycPop_Folder1"), L"", m_CycPop_Folder1, _countof(m_CycPop_Folder1));
		GetBufferedProfileString(pBuf, TEXT("Snow_CycPop_Folder2"), L"", m_CycPop_Folder2, _countof(m_CycPop_Folder2));
		GetBufferedProfileString(pBuf, TEXT("Snow_CycPop_Folder3"), L"", m_CycPop_Folder3, _countof(m_CycPop_Folder3));
		m_CycPop_ShowFolder = GetBufferedProfileInt(pBuf, TEXT("Snow_CycPop_ShowFolder"), 0) != 0;
		m_CycPop_ShowMp4 = GetBufferedProfileInt(pBuf, TEXT("Snow_CycPop_ShowMp4"), 0) != 0;
		m_CycPop_LastSelect = GetBufferedProfileInt(pBuf, TEXT("Snow_CycPop_LastSelect"), 0);
		/* 80% - 100% - 300% */
		m_Enlarge_ButtonWidth = GetBufferedProfileInt(pBuf, TEXT("Snow_EnlargeButtonWidth"), 100);
	}
...
void CTvtPlay::SaveSettings(bool fWriteDefault) const
{
	...
	/*SNOW*/
	{
		WritePrivateProfileInt(SETTINGS, TEXT("Snow_CycPop_LastSelect"), m_CycPop_LastSelect, m_szIniFileName);
		WritePrivateProfileInt(SETTINGS, TEXT("Snow_EnlargeButtonWidth"), m_Enlarge_ButtonWidth, m_szIniFileName);
	}
}
...
bool CTvtPlay::InitializePlugin()
{
	...
	for (int i = -1; i < BUTTON_MAX + 2; i++) {
		...
		int cmdID[2] = { -1, -1 };
		int cmdCount = 0;// , width = ICON_SIZE * m_ButtonWidthMulti;
		int width = static_cast<int>(1.0 * ICON_SIZE * m_Enlarge_ButtonWidth / 100);
	...
	/*ボタンの大きさの初期値を取得*/
	GetDefaltButtonWidth();
	SetButtonWidth(m_Enlarge_ButtonWidth);
	/*ポップアップ用のフォルダを追加*/
	std::wstring recFolder;
	TCHAR ini[MAX_PATH] = {};
	if (m_pApp->GetSetting(L"RecordFolder", ini, _countof(ini)) > 0)
		recFolder = std::wstring(ini);
	std::wstring f1 = std::regex_replace(m_CycPop_Folder1, std::wregex(L"%RecordFolder%"), recFolder);
	std::wstring f2 = std::regex_replace(m_CycPop_Folder2, std::wregex(L"%RecordFolder%"), recFolder);
	std::wstring f3 = std::regex_replace(m_CycPop_Folder3, std::wregex(L"%RecordFolder%"), recFolder);
	std::vector<std::wstring> f123{ f1, f2, f3 };
	m_cyclePop.Init(f123, m_CycPop_ShowFolder, m_CycPop_ShowMp4);
...
namespace Snow
{
	...
	{
	public:
		static MENUITEMINFO EnlargeButton(int magni)
		{
			TCHAR c[32];
			::swprintf_s(c, TEXT("%3.1f"), 1.0 * magni / 100);
			std::wstring cur = L" Width:  x " + std::wstring(c);

			HMENU hmenu = CreatePopupMenu();
			MENUITEMINFO mii;
			mii = { sizeof(MENUITEMINFO), MIIM_ID | MIIM_TYPE | MIIM_SUBMENU,
					 MFT_STRING, 0, 0, hmenu };
			mii.dwTypeData = L"Set Button Width";
			::AppendMenu(hmenu, MF_STRING | MF_GRAYED, 0, cur.c_str());
			::AppendMenu(hmenu, MF_SEPARATOR, 0, 0);
			::AppendMenu(hmenu, MF_STRING, OFFSET + W080, TEXT("         x 0.8"));
			::AppendMenu(hmenu, MF_STRING, OFFSET + W100, TEXT("         x 1.0"));
			::AppendMenu(hmenu, MF_STRING, OFFSET + W120, TEXT("         x 1.2"));
			::AppendMenu(hmenu, MF_STRING, OFFSET + W140, TEXT("         x 1.4"));
			return mii;
		}
	};
}
...
void CTvtPlay::SetupWithPopup(const POINT &pt, UINT flags)
{
	...
	if (hTopMenu) {
		...
		/*ボタンの幅*/
		int magni = m_Enlarge_ButtonWidth;
		MENUITEMINFO miBtn = Snow::MenuItemMaker::EnlargeButton(magni);
		InsertMenuItem(hmenu, UINT32_MAX, TRUE, &miBtn);
	...
	if (Snow::IDM::OFFSET <= selID) {
		int magni = selID - Snow::IDM::OFFSET;
		m_Enlarge_ButtonWidth = magni;
		SetButtonWidth(magni);
	}
}
...
bool CTvtPlay::OpenWithPopup(const POINT &pt, UINT flags)
{
	...
	HMENU hmenu;
	while (true) {
		m_cyclePop.UpdateFolder(path_playnow);
		m_cyclePop.CreateMenu(hmenu, path_playnow);
		if (hmenu) {
			int	selID = TrackPopup(hmenu, pt, flags);
			if (selID == CycPop::CycID::Cancel) {
				break;
			}
			else if (selID == CycPop::CycID::SwitchFolder) {
				m_CycPop_LastSelect = m_cyclePop.SwitchFolder();
				continue;
			}
			else if (selID == CycPop::CycID::NextPage) {
				m_cyclePop.NextPage();
				continue;
			}
			else {
				bool _continue;
				next = m_cyclePop.SelectMenu(selID, _continue);
				if (_continue)
					continue;
				else
					break;
	...
	if (hmenu)
		::DestroyMenu(hmenu);
	if (next.empty() == false)
		return m_playlist.PushBack(next.c_str(), true, true) >= 0 ? OpenCurrent() : false;
	else
		return false;
}
...
void CTvtPlay::Chapter_Insert(std::pair<int, CChapterMap::CHAPTER> ch, int erase)
{
	m_chapter.Insert(ch, erase);
}
void CTvtPlay::Chapter_Move(int from, int to)
{
	m_chapter.Move(from, to);
}
void CTvtPlay::Chapter_Erase(int pos)
{
	m_chapter.Erase(pos);
}
...
bool CTvtPlay::OpenCurrent(int offset, int stretchID)
{
	...
	bool isopen = !m_playlist.Get().empty() ? Open(m_playlist.Get()[m_playlist.GetPosition()].path, offset, stretchID) : false;
	SetAlwaysOnTop(isopen, true);
	return isopen;
}
...
{
	if (IsValidTvtpDriver() == false)/*地上波視聴時のシークバー操作を防止*/
		return;
...
/*sno*/
void CTvtPlay::SeekAndPreview(int msec)
{
	if (IsValidTvtpDriver() == false)/*地上波視聴時のシークバー操作を防止*/
		return;
	if (IsPaused())
		WaitAndPostToSender(WM_TS_SEEK_AND_PREVIEW, 0, msec, m_resetMode != 0);
	else
...
void CTvtPlay::BeginWatchingNextChapter(bool fDoDelay)
{
	if (m_hThread && !m_chapter.Get().empty()) {
		if (fDoDelay) {
			...
			int pos = GetPosition();
			/*
			０秒にシークしたときにスキップ処理を適用。
			０秒にシークするとpos=1300が返され c-0dix- が処理できない。
			2000以下に最初のスキップがあれば適用し、
			スキップ先が 2000以下だとループするので_2nd_posを考慮する。
			*/
			if (0 <= pos && pos < 2000) {
				std::map<int, CChapterMap::CHAPTER>::const_iterator _1st, _2nd;
				_1st = m_chapter.Get().begin();
				if (_1st != m_chapter.Get().end())
					_2nd = std::next(_1st, 1);
				if (_1st != m_chapter.Get().end() && _2nd != m_chapter.Get().end())
				{
					bool isBegin = _1st->second.IsSkipBegin();
					bool isEnd = _2nd->second.IsSkipEnd();
					int _1st_pos = _1st->first;
					int _2nd_pos = _2nd->first;
					if (m_fSkipXChapter && isBegin && isEnd && _1st_pos < 2000 && 2000 <= _2nd_pos) {
...
/*SetAlwaysOnTop*/
/*
   m_HaltCount_SetAlwaysOnTop:　一時的にSetAlwaysOnTop()を無効にする
	 同じコマンドを短時間で連続実行するとチェックボックスにだけ反映され最前面にならなかった。
	  -                        OpenCurrent() , ForceEnablePlugin(), Open(), Close()
	  - 起動直後の             EnablePlugin(), OpenCurrent()
	  - WM_QUERY_CLOSE_NEXTの  Close()       , OpenCurrent()
	  でのSetAlwaysOnTop()連打を無効にしている。
*/
...
{
	const std::wstring dlls[] = { L"BonDriver_UDP.dll", L"BonDriver_Pipe.dll" };
	TCHAR path[MAX_PATH];
	m_pApp->GetDriverName(path, _countof(path));
	LPCTSTR inUse = ::PathFindFileName(path);
	for (auto dll : dlls) {
		if (::lstrcmpi(inUse, dll.c_str()) == 0)
			return true;
	}
	return false;
}
...
void CTvtPlay::OnCommand(int id, const POINT *pPt, UINT flags)
{
	...
	{
		const int ID_COM[] = {
			ID_COMMAND_OPEN ,ID_COMMAND_OPEN_POPUP,
			ID_COMMAND_LIST_POPUP,ID_COMMAND_CLOSE };
		bool isOpenCmd = false;
		for (auto id_com : ID_COM)
		{
			if (id_com == id)
				isOpenCmd = true;
		}
	...
	case ID_COMMAND_SEEK_PREV:
		if (!m_chapter.Get().empty()) {
			...
			if (pos >= 3000) {
				...
				std::map<int, CChapterMap::CHAPTER>::const_iterator
					it = m_chapter.Get().lower_bound(pos - 3000);
				--it;
				/*"cix"なら無視、前の "cox"または、"c"までジャンプ*/
				for (;;)
				{
					if (it == m_chapter.Get().end()) break;
					else if (it == m_chapter.Get().begin() && !it->second.IsIn()) break;
				...
				if (it != m_chapter.Get().end())
					...
					SeekAbsolute(0); /*チャプターがなければ始端へ移動*/
			}
		}
		break;
		...
		if (!m_chapter.Get().empty()) {
			...
			if (pos >= 0) {
				...
				if (it != m_chapter.Get().end())
					...
					SeekAbsolute(CChapterMap::CHAPTER_POS_MAX); /*チャプターがなければ終端へ移動*/
			}
		}
		break;
...
/*ボタンの大きさの初期値を取得*/
void CTvtPlay::GetDefaltButtonWidth()
{
	...
	for (int i = 0; i < num; i++)
	{
		...
		if (pItem) {
			...
			if (STATUS_ITEM_BUTTON <= pItem->GetID())
				m_defaltButtonWidth.emplace(id, w);
		}
	}
}
/*ボタンの大きさを倍率をつけて設定*/
void CTvtPlay::SetButtonWidth(int magni = 100)
{
	int num = m_statusView.NumItems();
	for (int i = 0; i < num; i++)
	{
		...
		if (pItem) {
			int id = pItem->GetID();
			auto it = m_defaltButtonWidth.find(id);
			int w = it != m_defaltButtonWidth.end() ? it->second : ICON_SIZE;
			w = static_cast<int>(w * magni / 100);
			if (STATUS_ITEM_BUTTON <= pItem->GetID())
				pItem->SetWidth(w);
		}
	}
...
LRESULT CALLBACK CTvtPlay::EventCallback(UINT Event, LPARAM lParam1, LPARAM lParam2, void *pClientData)
{
	...
	case TVTest::EVENT_DRIVERCHANGE:
		...
		//ドライバが変更された
		{
			...
			if (pThis->IsValidTvtpDriver() == false) {
				...
				if (pThis->m_pApp->IsPluginEnabled()
					&& pThis->m_fHasBackup_AlwaysOnTop == false) {
					pThis->m_fBackup_AlwaysOnTop = pThis->m_pApp->GetAlwaysOnTop();
					pThis->m_fHasBackup_AlwaysOnTop = true;
				}
				//再生再開
				if (pThis->m_pApp->IsPluginEnabled()
		...
		// 全画面表示状態が変化した
	{
		...
		//bool isfull = pThis->m_pApp->GetFullscreen();
		//std::wstring mes = L"  ";
		//mes += L"fullscreen = ";
		//mes += std::to_wstring(isfull);
		//mes += L"\r\n";
	...
	case TVTest::EVENT_STARTUPDONE:
	{
		...
		*/
		...
		if (pThis->m_fForceEnable && pThis->m_szSpecFileName[0]) {
			if (pThis->m_playlist.PushBack(pThis->m_szSpecFileName, true, true) >= 0) {
		...
		/*TvtpAutoPlay*/
			...
			if (pThis->m_playlist.PushBack(ptn0.c_str(), true, true) >= 0) {
				isOpen = pThis->OpenCurrent(pThis->m_specOffset, pThis->m_specStretchID);
			}
		}
		pThis->m_HaltCount_SetAlwaysOnTop--;
...
BOOL CALLBACK CTvtPlay::WindowMsgCallback(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT *pResult, void *pUserData)
{
	...
	case WM_SETCURSOR:
	{
		////HWND hwnd2 = dynamic_cast<HWND>(wParam);
		//HWND hwnd2 = (HWND)(wParam);
		//LPARAM nHitTest = lParam & 0xFFFF;
		//LPARAM wMouseMsg = (lParam >> 16) & 0xFFFF;

		//std::wstring mes = L"  ";
		//mes += std::to_wstring(GetTickCount()) + L":  ";
		//mes += std::to_wstring(nHitTest) + L"  ";
		//mes += std::to_wstring(wMouseMsg) + L"  ";
		//mes += L"\r\n";
		//OutputDebugString(mes.c_str());
		break;
	}
	...
	//{
	//	std::wstring mes = L"  ";
	//	mes += std::to_wstring(GetTickCount()) + L":es  ";
	//	mes += L"\r\n";
	//	OutputDebugString(mes.c_str());
	//	break;
	//}
	...
	case WM_DROPFILES:
	{
		...
		for (int i = 0; i < num; ++i) {
			if (::DragQueryFile((HDROP)wParam, i, fileName, _countof(fileName)) != 0 &&
				...
				/*　media_count==1  -->  フォルダプレイ*/
				/*  複数 or list    -->  ドロップされたファイルだけ追加*/
				if (media_count == 1) {
					if (pThis->m_playlist.PushBack(fileName, !fAdded, true) >= 0)
						fAdded = true;
				}
				else {
					if (pThis->m_playlist.PushBack(fileName, !fAdded, false) >= 0)
						fAdded = true;
				}
