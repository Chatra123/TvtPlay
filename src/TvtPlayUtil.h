...
public:
	...
	/*chapter*/
public:
	virtual void Chapter_Insert(std::pair<int, CChapterMap::CHAPTER> ch, int erase) = 0;
	virtual void Chapter_Move(int from, int to) = 0;
	virtual void Chapter_Erase(int pos) = 0;
	virtual void BeginWatchingNextChapter(bool fDoDelay) = 0;
	/*GUI*/
	virtual int TrackPopup(HMENU hmenu, const POINT &pt, UINT flags) = 0;
	virtual void SeekAndPreview(int msec) = 0;
};
...
private:
	...
	/*Snow*/
	std::unique_ptr<Snow::CChapterOnSeekBar> m_spChp;
	std::unique_ptr<Snow::CDrawOnSeekBar> m_drawer;
	std::unique_ptr<Snow::CPopupOnSeekBar> m_popup;
};
