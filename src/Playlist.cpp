...
*/
int CPlaylist::PushBack(LPCTSTR path, bool fMovePos, bool fFolderPlay)
{
	// カレントからの絶対パスに変換
	TCHAR fullpath[MAX_PATH];
	DWORD rv = ::GetFullPathName(path, _countof(fullpath), fullpath, NULL);
	...
	else if (PathIsDirectory(fullpath) && fFolderPlay) {
		/*folder*/
		pos = PushBackCollectedFiles(fullpath, ListMax);
	}
...
void CPlaylist::Sort(SORT_MODE mode)
{
	...
	if (mode == SORT_ASC || mode == SORT_DESC) {
		...
		std::shuffle(sortList.begin(), sortList.end(), get_rand_mt);
	}
	m_list.clear();
	size_t pos = m_pos;
