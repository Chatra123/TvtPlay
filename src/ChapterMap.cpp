...
bool CChapterMap::Insert(std::pair<int, CHAPTER> &ch, int erase_pos)
{
	if (IsOpen()) {
		erase_pos = erase_pos != -1 ? erase_pos : ch.first;
		if (0 <= ch.first && ch.first <= CHAPTER_POS_MAX) {
			m_map.erase(erase_pos);
			m_map.insert(ch);
			Save();
			return true;
		}
...
bool CChapterMap::Insert_obs(const std::pair<int, CHAPTER> &ch, int pos)
{
...
bool CChapterMap::Move(int from, int to)
{
	if (IsOpen()) {
		std::pair<int, CChapterMap::CHAPTER> ch;
		{
			std::map<int, CChapterMap::CHAPTER>::const_iterator	it;
			it = m_map.find(from);
			if (it == m_map.end())
				return false;
			ch = std::pair<int, CChapterMap::CHAPTER>(*it);
			ch.first = to;
		}
		if (0 <= ch.first && ch.first <= CHAPTER_POS_MAX) {
			m_map.erase(from);
...
int CChapterMap::Clamp(int pos)
{
	return std::clamp(pos, 0, CHAPTER_POS_MAX);
}

void CChapterMap::Clamp(std::pair<int, CHAPTER> &ch)
{
	ch.first = std::clamp(ch.first, 0, CHAPTER_POS_MAX);
}
...
bool CChapterMap::InsertFrameStyleCommand(LPCTSTR p)
{
	...
	// time [msec]  -->  CHAPTER
	const wchar_t* IX = CChapterMap::CHAPTER::IX;
	const wchar_t* OX = CChapterMap::CHAPTER::OX;
	for (size_t i = 0; i < time.size(); i++)
	{
		...
		if (i == 0 && msec != 0) {
			...
			ch_i.first = 0;
			ch_i.second.name.assign(IX, IX + ::lstrlen(IX) + 1);
			m_map.insert(ch_i);

			std::pair<int, CHAPTER> ch_o;  // 1st cox
			ch_o.first = msec;
			ch_o.second.name.assign(OX, OX + ::lstrlen(OX) + 1);
			m_map.insert(ch_o);
		}
		else if (i % 2 == 0) {
