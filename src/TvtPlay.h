...
class CTvtPlay : public TVTest::CTVTestPlugin, public ITvtPlayController
{
	...
	static const int TIMER_ALWAYS_ON_TOP_INTERVAL = 2 * 1000;
...
private:
	...
	void UpdateFileInfoSetting(const HASH_INFO &hashInfo);
...
private:
	...
	bool m_fAutoPlay;
	/*Snow  FullScreen*/
	std::map<int, int> m_defaltButtonWidth;
	void GetDefaltButtonWidth();
	void SetButtonWidth(int magni);
	//int GetButtonItemFont();
	//void SetButtonItemFont(int size);
	/*Snow  Popup file*/
	Snow::CycPop6::CyclePopMenu m_cyclePop;
	TCHAR m_CycPop_Folder1[MAX_PATH];
	TCHAR m_CycPop_Folder2[MAX_PATH];
	TCHAR m_CycPop_Folder3[MAX_PATH];
	bool m_CycPop_ShowFolder;
	bool m_CycPop_ShowMp4;
	int m_CycPop_LastSelect;
	int m_Enlarge_ButtonWidth;
	/*Snow  chapter*/
	void Chapter_Insert(std::pair<int, CChapterMap::CHAPTER> ch, int erase);
	void Chapter_Move(int from, int to);
	void Chapter_Erase(int pos);

	void SeekAndPreview(int msec);
